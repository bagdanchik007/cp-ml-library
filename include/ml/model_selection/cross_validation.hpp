...
inline std::vector<double> matrix_to_vector(
    const Matrix& matrix
) {
    if (matrix.cols != 1) {
        throw std::invalid_argument(
            "matrix_to_vector: matrix must have exactly one column"
        );
    }
...
std::vector<double> cross_validate(
    ...
    if (features.rows != targets.size()) {
        throw std::invalid_argument(
            "cross_validate: feature and target row counts differ"
        );
    }
    ...
    for (const auto& fold : splitter.split(features.rows)) {
        ...
        const Matrix x_train =
            select_rows(features, fold.train);

        const Matrix x_test =
            select_rows(features, fold.test);

        const auto y_train =
            select_rows(targets, fold.train);

        const auto y_test =
            select_rows(targets, fold.test);
        ...
        const auto predictions =
            matrix_to_vector(predictions_matrix);

        scores.push_back(
            scorer(y_test, predictions)
        );
    }
